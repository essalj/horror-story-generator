----------------------
Master prompt for LLM Horror Story Generation Recipe
----------------------
Instructions:
...
Word Count: The story must be a minimum of 1200 words. Prioritize detailed descriptions, character development, and escalating tension to meet this requirement.
...
Storyteller Gender: Clearly state the gender of the storyteller in the output.
...
When a character is present, include their full physical bio in the prompt to ensure consistency across images.
Emphasize elements that enhance the horror, such as shadows, distorted perspectives, and unsettling details.
7. Structured Output
ABSOLUTELY CRITICAL - STRICT JSON OUTPUT: The final output MUST be a single JSON object adhering strictly to the specified format. No other output format, introductory text, concluding remarks, or extraneous text outside the JSON structure is acceptable. Only the JSON object is permitted.
Output all generated content in a single JSON object with the following structure:
json
Copy Code
{
  "genre_inspiration": {
    "popular_stories": [
      {"title": "The Shining", "description": "A family is terrorized by supernatural forces in an isolated hotel."},
      {"title": "The Russian Sleep Experiment", "description": "A Creepypasta tale about a horrifying experiment gone wrong in a Soviet lab."},
      {"title": "The Smiling Man (Reddit/NoSleep)", "description": "A chilling encounter with a grinning stranger on a deserted street."},
      ...
    ]
  },
  "pitches": [
    {"pitch": "A...", "score": 87, "justification": "Very original..."},
    ...
  ],
  "selected_pitch": {
    "pitch": "A...",
    "score": 92,
    "justification": "The most intense and original..."
  },
  "character_bios": [
    {
      "name": "Emily Carter",
      "age": "16",
      "gender": "Female",
      "appearance": "Long brown hair, green eyes, slim build, always wears a red hoodie, has a scar on her left cheek."
    },
    ...
  ],
  "storyteller_gender": "Male",
...
Minimum Word Count: The story MUST be at least 1200 words long.
...
Storyteller Gender: The storyteller's gender MUST be clearly stated in the storyteller_gender field of the JSON output.
...
Genre Inspiration Source Mix: The "Genre Inspiration" section MUST include 5 examples from classic/mainstream media and 5 from internet-based platforms.
...
LAST AND CRUCIAL STEP: 
Your task is to act as an expert JSON fixer. Repair any broken JSON below, correcting all syntax errors. Output only the corrected JSON, ready for parsing with `json.loads()` in Python. Do not include any code fences, backticks, explanations, or extra text—just the raw JSON. NOTHING ELSE WILL BE ACCEPTED!! I WANT THE FIRST CHAR TO BE '{' AND I WANT THE LAST CHAR TO BE '}'